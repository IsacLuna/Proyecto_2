...
void calcDeriv(float ecuacion[], float primDeriv[], float segDeriv[],int grado, int grado1, int grado2);
int iteraciones(float ecuacion[], float primDeriv[], float segDeriv[], int grado, int grado1, int grado2);
...
int main(int argc, char const *argv[]){
    ...
    do{
	    ...
	    grado2 = grado1 - 1;
	    float derivada2[grado2];
	    calcDeriv(ecuacion, derivada, derivada2, grado, grado1, grado2);
	    val = iteraciones(ecuacion, derivada, derivada2, grado, grado1, grado2);	
	}while(val == 1);
	printf("\n\n\n\n\t\t\t\t----------------%cGRACIAS%c----------------\n\n", 173,33);
    return 0;
}
...
int iteraciones(float ecuacion[], float primDeriv[], float segDeriv[], int grado, int grado1, int grado2){
	...
	do{
		...
		if(valor1 == 0){
			printf("Se est%c realizando un división entre cero. ", 160);
			break;
		}
        for(i = grado2; i >= 0; i--){
            valor2 = valor2 + segDeriv[i]*pow(xInicial, i); 
        }
        sigX = xInicial - (valor*valor1/(pow(valor1, 2)-(valor*valor2)));
